<template>
  <div class="makeup-clock-approval">
    <el-card>
      <template #header>
        <span>补卡审批</span>
      </template>
      ...
      <el-table :data="records" border stripe>
        ...
        <el-table-column prop="reason" label="补卡原因" />
...
import { getMakeupClockList, approveMakeupClock, rejectMakeupClock } from '@/api/makeup-clock'
...
async function loadData() {
  try {
    const res = await getMakeupClockList({
    ...
    console.error('获取补卡列表失败', error)
  }
}

async function handleApprove(row: any) {
  try {
    await approveMakeupClock(row.id)
...
async function confirmReject() {
  ...
  try {
    await rejectMakeupClock(currentId.value!, { rejectReason: rejectReason.value })
